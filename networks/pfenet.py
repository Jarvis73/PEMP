...
from networks import pfe_resent as models
from networks import backbones

pretrained_weights = {
    "resnet50v2": Path(__file__).parents[1] / "data/resnet50_v2.pth",
}
...
class PFENet(backbones.BaseModel, nn.Module):
    def __init__(self, shot, logger):
        ...
        resnet = models.resnet50(pretrained=True, path=pretrained_weights["resnet50v2"])
    ...
    def forward(self, sup_img, sup_mask, qry_img, qry_mask, out_shape=None):
        x = qry_img.squeeze(dim=1)  # [B, 3, H, W]
        y = qry_mask.squeeze(dim=1) # [B, H, W]
        s_x = sup_img               # [B, S, 3, H, W]
        s_y = sup_mask[:, :, 0]     # [B, S, H, W]
        ...
        for idx, tmp_bin in enumerate(self.pyramid_bins):
            if tmp_bin <= 1.0:
                bin_ = int(query_feat.shape[2] * tmp_bin)
                query_feat_bin = nn.AdaptiveAvgPool2d(bin_)(query_feat)
            else:
                bin_ = tmp_bin
                query_feat_bin = self.avgpool_list[idx](query_feat)
            supp_feat_bin = supp_feat.expand(-1, -1, bin_, bin_)
            corr_mask_bin = F.interpolate(corr_query_mask, size=(bin_, bin_), mode='bilinear', align_corners=True)
        ...
        if out_shape is None:
            out_shape = (h, w)
        out = F.interpolate(out, size=out_shape, mode='bilinear', align_corners=True)
        ...
        if self.training:
            ...
            for idx_k in range(len(out_list)):
                ...
                inner_out = F.interpolate(inner_out, size=out_shape, mode='bilinear', align_corners=True)
